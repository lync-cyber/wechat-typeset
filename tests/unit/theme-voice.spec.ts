...
describe('主题 voice · 签名容器缺独有 voice 告警', () => {
  it('signatureContainer 已登记但 spec.containers.<key> 缺失 → warning', () => {
    // data-brief 派生 spec：剥掉 toc 槽位以验证规则触发面。
    // 通过克隆而非依赖某主题"恰好缺槽"——保证规则契约不被未来主题补全悄悄掩盖。
    const base = LOADED.find((l) => l.dir === 'data-brief')!.spec
    const stripped: PersonaSpec = {
      ...base,
      containers: Object.fromEntries(
        Object.entries(base.containers ?? {}).filter(([k]) => k !== 'toc'),
      ),
    }
    const r = analyzeThemeVoice(stripped)
    const hit = r.warnings.find((w) => /signatureContainers\.toc/.test(w.path))
