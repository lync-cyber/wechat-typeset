 ...
 * 渲染器"分支单独处理，不走这条路。
 */
import type { UserVariantPatch, UserVariantTokens, UserVariantCssSlot } from '../../../variants/userVariant';
import type { VariantRenderResult } from '../../../variants/_core';
export type AppliableUserVariant = UserVariantTokens | UserVariantPatch;
export declare function applyUserVariant(uv: AppliableUserVariant, base: VariantRenderResult): VariantRenderResult;
/**
 ...
 * tokens 为空 → 原 css 不动。
 */
export declare const USER_TOKEN_PREFIX = "--uv-";
/** 给外部测试 / 调试用：判断 uv 是否在 makeVariantContainer 的支持范围。 */
export declare function isAppliableUserVariant(uv: {
    level: string;
}): uv is AppliableUserVariant;
/** 给 makeVariantContainer 用：枚举可被 patch 的 CSS 槽位（与 UserVariantCssSlot 同步）。 */
export declare const PATCHABLE_SLOTS: ReadonlyArray<UserVariantCssSlot>;
