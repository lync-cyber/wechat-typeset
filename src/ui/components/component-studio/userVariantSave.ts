...
export function dispatchUserVariant(
  ...
  if (effectiveOrig && isBaseValid(draft)) {
    ...
    if (cur) {
      ...
      if (baseDrifted || levelDrifted) {
        // 把 markdown 里 variant=<orig.id> 改回 draft.variantId（新 base），让 case 3 能正确改写
  ...
  if (!effectiveOrig && hasUvContent(draft)) {
    ...
    if (input) {
      ...
      const rewritten = rewriteVariantInMarkdown(mdAfterDriftReset, draft.variantId, uv.id)
